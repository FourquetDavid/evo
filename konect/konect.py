...
if __name__ == '__main__':
    ...
    def bipartite_konect(graph_init,name,type0,type1,trois=None,quatre=None):
        ...
        graph.name = name
        spname = name.split("_")
        little_name="_"+spname[-1]
        if len(spname) == 1 : 
            little_name = ""  
            
        for line in (open(name+"/out."+graph.name+little_name).read().splitlines()) :
                ...
                if nodes_info < 10000 :
                    ...
                    try :
                        if not os.path.exists(name+".tar.bz2") and not os.path.exists(name) :
                            urllib.urlretrieve ("http://konect.uni-koblenz.de/downloads/tsv/"+name+".tar.bz2", name+".tar.bz2")
