...
def getDataFromKonect():
    ht = html.parse("konect.html")
    table = ht.find('body/div/table/tbody')
    for network in table.iter('tr'):

        try:
            name = network[1][0].get("href").replace("http://konect.uni-koblenz.de/networks/", "")
        except:
            continue
        ...
        signed = "Signed" in network[4][1][0].get("alt") or "atings" in network[4][1][0].get("alt")
        ...
        try:
            time_info = False
            time_info = network[5][0][3][0] is not None
        except:
            pass
...
def getNetworksDone():
    ...
    if os.path.isfile("done.csv"):
        ...
        networkfile.write(
            "number;name;nb_nodes;nb_edges;is_directed;has_multiple_edges;is_bipartite;is_weighted;is_signed;is_dynamic;number_of_node_datas;list_node_datas;number_of_edge_datas;list_edge_datas;\n")
    networkfile.close()
    return networks
...
def compareDataswithFiles():
    def study(carac):
        ...
        if type(expected_datas[carac]) == list or type(found_datas[carac]) == list:
            if set(expected_datas[carac]) == set(found_datas[carac]):
                ...
                expected_datas['csv'] = expected_datas["csv"] + '_'.join(expected_datas[carac]) + "/" + '_'.join(
                    found_datas[carac]) + ";"
        ...
        if expected_datas[carac] != found_datas[carac]:
            print carac, "expected:", expected_datas[carac], "found:", found_datas[carac]
            expected_datas['csv'] = expected_datas["csv"] + str(expected_datas[carac]) + "/" + str(
                found_datas[carac]) + ";"
            return False
        else:
    ...
    for directory in netfiles:
        ...
        if os.path.isdir("files/" + directory) and (directory.split("-", 1)[1] not in networks) and (
                    directory.split("-", 1)[1] not in net2):
            ...
            perfect = study("nb_loops") and perfect
...
def network_description(directory):
    ...
    def is_multiple():
        ...
        network_datas["nb_loops"] = 0
        if isinstance(net, nx.MultiGraph) or isinstance(net, nx.MultiDiGraph):
            if net.number_of_selfloops() > 0:
                network_datas["has_loop"] = True
                network_datas["nb_loops"] = net.number_of_selfloops()
...
# for a,b in dict_id_datas.iteritems() :
# print(a)
# print(b)
