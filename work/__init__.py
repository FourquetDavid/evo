...
# import igraph as ig
def write(net, f, number):
    name = f.split('.')[-2]
    net.name = name
    if not os.path.exists("files/" + str(number) + "-" + name): os.makedirs("files/" + str(number) + "-" + name)
    nx.write_gexf(net, "files/" + str(number) + "-" + name + "/" + name + ".gexf")
    """
...
def verif(f, number):
    name = f.split('.')[-2]
    net = nx.read_gexf("files/" + str(number) + "-" + name + "/" + name + ".gexf")
    ...
    for f in os.listdir('/files'):
        try:
            if os.path.isdir(f):
                ...
                output.write(
                    idnetwork + ";" + namenetwork + ";" + nbnodes + ";" + nbedges + ";" + is_directed + ";" + is_weighted + ";" + is_signed + ";" + is_multi + ";" + is_dynamic)
    ...
    for source, target, data in netslice.edges(data=True):
        ...
        for data_type, value in data.iteritems():
            try:
                ...
                print "data not found : " + data_type + " edge : " + str(source) + " " + str(target) + " time: " + str(
                    currenttime)
...
def VRND():
    ...
    def subgraph(number_time):
        ...
        for line in open("VRND/VRND32T" + str(number_time) + ".DAT").read().splitlines():
            ...
            for number_of_messages in [int(number) for number in line.split(" ") if number != '']:
                if number_of_messages not in [0,6,9]:
    ...
    # test(net)
    # test(subgraph(0))
    net = add_following_slice(net, subgraph(0), 0, 0)

    net = add_following_slice(net, subgraph(1), 0, 1)
    # test(net)
    net = add_following_slice(net, subgraph(2), 1, 2)
    net = add_following_slice(net, subgraph(3), 2, 3)
    net = add_following_slice(net, subgraph(4), 3, 4)
    net = add_following_slice(net, subgraph(5), 4, 5)
    net = add_following_slice(net, subgraph(5), 5, 6)
    #test(net)
    """
    #suivi(net, "VanDeBunt_students.gexf", dynamic=True)
    test(subgraph(0))
    suivi(subgraph(0), "VanDeBunt_students_0.gexf", 4)
    suivi(subgraph(1), "VanDeBunt_students_1.gexf", 0)
    suivi(subgraph(2), "VanDeBunt_students_2.gexf", 0)
    suivi(subgraph(3), "VanDeBunt_students_3.gexf", 0)
    suivi(subgraph(4), "VanDeBunt_students_4.gexf", 0)
    suivi(subgraph(5), "VanDeBunt_students_5.gexf", 0)
    suivi(subgraph(6), "VanDeBunt_students_6.gexf", 0)
...
def vand():
    ...
    def attribute(graph, number):
        ...
        for line in open("cov" + str(number) + ".dat").read().splitlines():
            ...
            smo = {99: "unknown", 9: "unknown", 0: "no", 1: "no", 2: "yes, at parties (social)",
                   3: "1-3 cigarettes per day", 4: "4-10 cigarettes per day", 5: "more than 10 p.d."}
            drug = {99: "unknown", 1: "no", 2: "yes, less than once a month", 3: "yes, 1-3 times p.m.",
                    4: "yes, 1-3 times per week", 5: "yes, more than 3 times p.w."}
            satisfaction = {99: "unknown", 1: "very dissatisfied", 2: "rather dissatisfied",
                            3: "not satisfied/not dissatisfied", 4: "rather satisfied", 5: "very satisfied"}
            numberf = {99: "unknown", 1: "far too few", 2: "too few", 3: "exactly right number", 4: "too many",
                       5: "far too many"}
            ...
            if number == 1:
                ...
                out, movie, dinner, exerc, watch, coffee, relig, listening, politics, personal, fun, assoc, classes = linesplit[
                                                                                                                      5:18]
    ...
    def subgraph(number_time):
        ...
        def type_rel(number):
            d = {0: "unknown", 1: "Best friendship", 2: "Friendship", 3: "Friendly relationship",
                 4: "Neutral relationship", 5: "Troubled relationship"}
            return d[number]
    ...
    net = add_following_slice(net, subgraph(1), 1, 1)
    # test(net)
    net = add_following_slice(net, subgraph(2), 1, 2)
    net = add_following_slice(net, subgraph(3), 2, 3)
    net = add_following_slice(net, subgraph(4), 3, 4)
    net = add_following_slice(net, subgraph(5), 4, 5)
    test(net)
    suivi(net, "vanDuijn_students.gexf")
...
def stu98():
    ...
    def subgraph(number_time):
        ...
        def type_rel(number):
            d = {1: "Best friendship", 2: "Friendship", 3: "Friendly relationship", 4: "Neutral relationship",
    ...
    net = add_following_slice(net, subgraph(0), 0,  0)
    # test(net)
    net = add_following_slice(net, subgraph(2), 0,  2)
    net = add_following_slice(net, subgraph(3),  2, 3)
    net = add_following_slice(net, subgraph(5),  3, 5)
    net = add_following_slice(net, subgraph(6),  4, 6)
    # test(net)
    #suivi(net, "stud98.gexf", 27,dynamic=True)
    suivi(subgraph(0),"stud98_0.gexf",0)
    suivi(subgraph(2),"stud98_2.gexf",0)
    suivi(subgraph(3),"stud98_3.gexf",0)
    suivi(subgraph(5),"stud98_5.gexf",0)
    suivi(subgraph(6),"stud98_6.gexf",0)
...
def EIES(name):
    ...
    def subgraph(number_time):
        ...
        def type_rel(number):
            d = {1: "do_not_know_the_other", 2: "heard_about_the_other,did_not_meet_him/her", 3: "have_met_the_other",
                 4: "friend"}
            return d[number]
...
def twitter(name,number):
    realname = name + "-twitter"
    graph = nx.MultiDiGraph()
    graph.name = realname
    graph1 = nx.DiGraph()
    graph2 = nx.DiGraph()
    def feature(graph,namefeature):
        dictfeatures = {}
        i = 0
    ...
    def add_nodes(graphe) :
        graphe.add_nodes_from(open(name + "/" + name + ".ids").read().splitlines())
        for line in open(name + "/" + name + ".communities").read().splitlines():
            ...
            for named in names:
                graphe.node[named]['community'] = community_name
                graphe.node[named]['listmerged500'] = {}
                graphe.node[named]['lists500'] = {}
                graphe.node[named]['tweets500'] = {}
        feature(graphe,"listmerged500")
        feature(graphe,"lists500")
        feature(graphe,"tweets500")
    add_nodes(graph1)
    add_nodes(graph2)
    def add_typed_edge(graphe,typedg):
        for line in open(name + "/" + name + "-" + typedg + ".mtx").read().splitlines()[1:]:
            debut, fin, size = line.split(" ")
            graphe.add_edge(debut, fin, type=typedg, weight=float(size))

    #add_typed_edge("followedby")
    add_typed_edge(graph1,"follows")
    #add_typed_edge("retweetedby")
    add_typed_edge(graph2,"retweets")
    ...
    graph1 = nx.convert_node_labels_to_integers(graph1)
    ...
    suivi(graph1, realname + "_follows.gexf",number)
    suivi(graph2, realname + "_retweets.gexf",0)
...
def lazega():
    ...
    def attribute():
        ...
        for line in open(name + "/" + "ELattr.dat").read().splitlines():
            ...
            seniority, status, gender, office, years, age, practice, school = [number for number in line.split(" ") if
                                                                               number != '']
...
def read_pajek(path, encoding='UTF-8'):
    ...
    return G
def read_food_pajek(path, encoding='UTF-8'):
    path2 = open(path, mode='rb').read().splitlines()
    lines = (line.decode(encoding) for line in path2)
    ...
    G = nx.DiGraph()
    category={}
    category_dict ={"1":"Living","2":"Other","3":"Input","4":"Output","5":"Respiration"}
    while lines:
        try:
            l = next(lines)
        except:  # EOF
            break
        if l.lower().startswith("*partition"):
            #categories
            l=next(lines)
            nb_vertices = int(l.split(" ")[1])
            for i in range(nb_vertices):
                l=next(lines)
                category[i] = category_dict[l]
        if l.lower().startswith("*network"):
            #name
            label, name = l.split()
            G.name = name
        if l.lower().startswith("*vertices"):
            ...
            for i in range(int(nnodes)):
                ...
                G.add_node(label)
                nodelabels[ida] = label
                G.node[label] = {'id': ida,'category':category[i]}
                extra_attr = zip(splitline[5::2], splitline[6::2])
                G.node[label].update(extra_attr)
        ...
        if l.lower().startswith("*vector"):
            l=next(lines)
            for i in range(nb_vertices):
                l=next(lines)
                G.node[i]["biomasse"]=float(l)
    ...
    while lines:
        ...
        if l.startswith("*Arcs"):
            while lines:
                ...
                net.add_edge(int(source), int(target))
                net[int(source)][int(target)]['edge_type'] = dict_type[int(type_relation)]
...
def read_ucinet(name):
    net = nx.Graph()
    realname = name.replace("_2.0", "").replace("-agent", "").replace("-organization", "").replace("-Agent",
                                                                                                   "").replace(
        "-location", "").replace('.dl', "")
    net.name = realname
        ...
        if l.startswith("DATA:"):
            ...
            while lines:
                ...
                if l.startswith("#"):
                    ...
                    ''' Verifie que le reseau est vraiment pondere (pas que des poids egaux a 1)'''
                    if not real_weighted:
                        for _, _, c in net.edges_iter(data=True):
                            if c.get('edge_type', None) is None or c['edge_type'] == current_network:
                                del c['weight']
                        real_weighted = False
                ...
                if int(value) != 1:
                    real_weighted = True
                if int(nb_of_networks) > 1:
                    net.add_edge(source, target, edge_type=current_network, weight=int(value))
                else:
...
def munmun():
    ...
    def add_edge(source, target, timestamp):
        ...
        if not graph.has_edge(source, target):
            graph.add_edge(source, target, weight=1, timestamp1=timestamp)
        else:
            edge = graph.get_edge_data(source, target)
            edge["weight"] += 1
            edge["timestamp" + str(edge["weight"])] = timestamp
...
def facebook(number, save):
    ...
    edges()
    circles()
    print "infos", graph.number_of_nodes()
    # print graph.edge[node]
    # print graph.node[node]['circle']
    suivi(graph, graph.name + ".gexf", str(save))
...
def consulting():
    ...
    graph1.name = realname
    graph2.name = realname
    def edge(graph,nameE, nameExt, dico):
    ...
    edge(graph1,"info", "ask_for_advice",
         {0: "I Do Not Know This Person", 1: "Never", 2: "Seldom", 3: "Sometimes", 4: "Often", 5: "Very Often"})
    edge(graph2,"value", "has_expertise",
         {0: "I Do Not Know This Person", 1: "Strongly Disagree", 2: "Disagree", 3: "Neutral", 4: "Agree",
          5: "Strongly Agree"})

    def attribute(graph,nameA, dico, realnameA=None):
        ...
        for line in open(name + "-" + nameA + ".txt").read().splitlines():
            ...
            for prop in [int(entier) for entier in linesplit if entier != '']:
                ...
                i += 1
    def atts(graphe):
        attribute(graphe,"gender", {1: "male", 2: 'female'})
        attribute(graphe,"orglevel",
                  {1: "Research Assistant", 2: "Junior Consultant", 3: "Senior Consultant", 4: "Managing Consultant",
                   5: "Partner"}, "organisational_level")
        attribute(graphe,"region", {1: "Europe", 2: "USA"})
        attribute(graphe,"location", {1: "Boston", 2: "London", 3: "Paris", 4: "Rome", 5: "Madrid", 6: "Oslo", 7: "Copenhagen"})
    atts(graph1)
    atts(graph2)
    # import random
    # print random.choice(graph.nodes(data=True))
    # print random.choice(graph.edges(keys=True,data = True))
    suivi(graph1, realname + ".gexf",47)
    suivi(graph2, realname + ".gexf",0)
...
def manufacturing():
    ...
    graph.name = realname
    graphe1 = nx.DiGraph()
    graphe1.name = realname
    graphe2 = nx.DiGraph()
    graphe2.name = realname
    def edge(graphe,nameE, nameExt, dico):
        for line in open(name + "_" + nameE + ".txt").read().splitlines():
            source, target, weight = [int(entier) for entier in line.split(" ") if entier not in['']]
            if weight != 0:
                graphe.add_edge(source, target, type=nameExt, **{nameExt: dico[weight]})

    edge(graphe1,"info", "provides_information",
         {0: "I Do Not Know This Person", 1: "Very Infrequently", 2: "Infrequently", 3: "Somewhat Infrequently",
          4: "Somewhat Frequently", 5: "Frequently", 6: "Very Frequently"})
    edge(graphe2,"aware", "aware_of_skill",
         {0: "I Do Not Know This Person", 1: "Strongly Disagree", 2: "Disagree", 3: "Somewhat Disagree",
    ...
    def attribute(graph,nameA, dico,realnameA = None):
        ...
        for line in open(name + "-" + nameA + ".txt").read().splitlines():
            linesplit = line.split(" ")
            i = 1
            for prop in [int(entier) for entier in linesplit if entier not in ['']]:
                graph.node[i][realnameA] = dico[prop]
                i += 1
    def atts(graphe) :
        attribute(graphe,"tenure", {1: "1-12 months", 2: '13-36 months', 3: "37-60 months", 4: "61+ months"})
        attribute(graphe,"orglevel", {1: "Global Dept Manager", 2: "Local Dept Manager", 3: "Project Leader", 4: "Researcher"},
                  "organisational_level")
        attribute(graphe,"location", {1: "Paris", 2: "Frankfurt", 3: "Warsaw", 4: "Geneva"})
    atts(graphe1)
    atts(graphe2)
    # import random
    # print random.choice(graph.nodes(data=True))
    # print random.choice(graph.edges(keys=True,data=True))
    suivi(graphe1, realname + "_info.gexf",49)
    suivi(graphe2, realname + "_aware.gexf",0)


def terrorAttack():
    name = "terrorist_attack"
    sitelabel = "http://counterterror.mindswap.org/2005/terrorism.owl#"
    site2 = "http://counterterror.mindswap.org/2005/ict_events.owl#"
    graph1 = nx.Graph()
    graph1.name = "terrorist_attack_colocated"
    graph2 = nx.Graph()
    graph2.name = "terrorist_attack_colocated_same_organization"

    def attribute(graph):

        for line in open("TerrorAttack/" + name + ".nodes").read().splitlines():
            linesplit = line.split("\t")
            nodename = linesplit[0].replace(site2,"")
            graph.add_node(nodename)
            i = 0
            for prop in linesplit[1:-1]:
                graph.node[nodename]["attribute" + str(i)] = int(prop)
                i += 1
            graph.node[nodename]['type'] = linesplit[-1].replace(sitelabel,"")

    attribute(graph1)
    attribute(graph2)

    import random
    print random.choice(graph1.nodes(data=True))

    def edges():
        for line in open("TerrorAttack/" + name + "_loc.edges").read().splitlines():
            source, target = line.split(" ")
            graph1.add_edge(source.replace(site2,""), target.replace(site2,""))
        for line in open("TerrorAttack/" + name + "_loc_org.edges").read().splitlines():
            source, target = line.split(" ")
            graph2.add_edge(source.replace(site2,""), target.replace(site2,""))

    edges()
    print random.choice(graph2.edges())
    suivi(graph1, "TerrorAttack_1.gexf",44)
    suivi(graph2, "TerrorAttack_2.gexf",0)
...
def test_konect():
    name = "elec"
    net = nx.read_gexf("files/0-" + name + "/" + name + ".gexf")

    '''
    user =0
    page =0
    for _,g in net.nodes(data = True) :
        if g['type'] == "User" :
            user+=1
        if g['type'] == " article" :
            page+=1
    ...
    multiple = 0
    for source, target in net.edges():
        if net.number_of_edges(source, target) > 1:
            print net.number_of_edges(source, target)
            multiple += 1
...
def advogato():
    net = nx.read_gexf("files/0-advogato/advogato.gexf")
    net2 = nx.DiGraph(net)
    print net2.number_of_nodes()
...
def USAir97():
    net = read_pajek("USAir97.net")
    net.name = "USAir97"
    #mapy = {oldkey: int(oldkey) for oldkey in net}
    #net = nx.relabel_nodes(net, mapy)

    test(net)
    suivi(net,"USAir97.gexf",0)

def foodwebs(dir):
    import os
    for file in os.listdir(dir):
        print dir+"/"+file
        net = read_food_pajek(dir+"/"+file)
        net.name = file.replace(".paj","")
        test(net)
        suivi(net,file.replace(".paj",".gexf"),0)
...
def jazz():
    ...
    net = nx.relabel_nodes(net, mapy)
    for source, target in net.edges():
        net[source][target].pop("weight", None)
    suivi(net, "jazz.net")
